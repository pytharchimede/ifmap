...
class Payment extends CI_Controller
{
    ...
    function success_instructor_payment($payment_method = "")
    {
        ...
        // Condition spécifique à CinetPay
        if ($payment_method === 'cinetpay' && !$response) {
            $this->load->model('addons/CinetPay_model');
            $response = $this->CinetPay_model->check_cinetpay_payment($payment_method, 'course');
        }
    ...
    function create_cinetpay_payment()
    {
        $payment_details = $this->session->userdata('payment_details');
        ...
        $this->session->set_userdata('payment_details', array_merge($payment_details, ['transaction_id' => $transaction_id]));
        ...
        $post_data = [
            ...
            'return_url' => site_url('payment/success_course_payment/cinetpay'),
            'notify_url' => site_url('payment/cinetpay_notify'),
        ];

        $url = $payment_gateway['enabled_test_mode'] ?
            'https://sandbox.cinetpay.com/api/v1/payment' :
            'https://api.cinetpay.com/api/v1/payment';
        ...
        if ($response && $response['code'] === '00') {
            ...
            $this->session->set_flashdata('error_message', site_phrase('unable_to_initiate_payment'));
            redirect('home/shopping_cart', 'refresh');
        }
    }


    private function make_curl_request($url, $post_data)
    {
    ...
    function cinetpay_notify()
    {
        $input = file_get_contents('php://input');
        $data = json_decode($input, true);

        if ($data && $data['code'] === '00') {
            // Mettre à jour la base de données pour marquer le paiement comme validé
            $this->crud_model->course_purchase($data['payer'], 'cinetpay', $data['amount']);
        }
    }
